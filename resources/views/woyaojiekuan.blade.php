        ...
        <div class="delCon">
            ...
            <div class="iptBox">
                ...
                <select class="ageBox" id="age" name="age" >
                    <option selected="selected" >请选择年龄段</option>
                    <option value="15">23岁以下</option>
                    <option value="40">23岁-55岁</option>
                    <option value="80">55岁以上</option>
                </select>
                @if($errors->has('age'))
                <p style="display: block;" id="ageError" class="error">{{$errors->first('age')}}</p>
                @endif
            </div>
            ...
            <div class="iptBox">
                ...
                <img src="{{url('captcha')}}" name="imgc" id="imgc" alt="换一张" class="get-code" height="36" width="80" onclick="this.src+='?'+Math.random()">
                @if(session('msg'))
            ...
            <div class="iptBox">
                ...
                <i><a class="get-code" id="dtmbtn" name="dtmbtn" >获取验证码</a></i>
                 @if(session('yzmpd'))
                <p id="imgcodeError" style="display: block;" class="error">{{session('yzmpd')}}</p>
                @endif
            </div>
...
<script type="text/javascript">
    $('#applyForm').submit(function(){
        ...
        if($('#loanAmount').val() == ''){
            alert('借款金额不能为空');
            return false;
        }else if(!patt.test($('#loanAmount').val())){
            alert('请输入正确格式的借款金额');
            return false;
